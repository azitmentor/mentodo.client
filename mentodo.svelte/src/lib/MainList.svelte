<script lang="ts">
  ...
  async function refresh() {
    const response = await fetch("http://todoapi.azitmentor.hu/todo");
...
<button class="btn btn-primary" on:click={() => refresh()}> Search </button>
Add new:
  ...
  <tbody>
    {#each itemList as item}
      <tr>
        <td>{item.id}</td>
        <td>
          <input type="checkbox" bind:checked={item.done} on:click={()=> doneClick(item.id)} />
        </td>
        <td>{item.info}</td>
        <td>{item.priority}</td>
        <td>
          <button class="btn btn-warning" on:click={() => deleteClick(item.id)}>
            Delete
          </button>
        </td>
      </tr>
